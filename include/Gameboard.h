...
class GameBoard {
    ...
    float difficulty;
    enum difficulties {easy = 100, normal = 75, hard = 50, stupid_hard = 25};
    int choice;
    int score;
...
GameBoard::GameBoard() {
    ...
    choice = proceed;
    score = 0;
    difficulty = snakeBody[0].waitTime;
    Apple apple;
}
...
void GameBoard::gui() {
    ...
    mousePosX = GetMouseX();
    mousePosY = GetMouseY();
    Rectangle buttonToggleDifficulty = {150, 800, 50, 100};
    if ((mousePosX > 150) && (mousePosX < 200) && (mousePosY > 800) && (mousePosY < 850)) {
        DrawRectangleRec(buttonToggleDifficulty, GREY);
    } else {
        DrawRectangleRec(buttonToggleDifficulty, WHITE);
    } 
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && 
    (mousePosX > 150) && (mousePosX < 200) && (mousePosY > 800) && (mousePosY < 850)) {
        if ((difficulty == (float)easy))
            difficulty = (float)normal;
        else if ((difficulty == (float)normal))
            difficulty = (float)hard;
        else if ((difficulty == (float)hard))
            difficulty = (float)stupid_hard;
        else
            difficulty = (float)easy;
    }
    snakeBody[0].waitTime = difficulty;
    if ((difficulty == (float)easy)) {
        DrawText("EASY", 155, 805, 16, BLACK);
    } else if ((difficulty == (float)normal)) {
        DrawText("NOR-\nMAL", 155, 805, 16, BLACK);
    } else if ((difficulty == (float)hard)){
        DrawText("HARD", 155, 805, 16, BLACK);
    } else {
       DrawText("STU-\nID\nHARD", 155, 805, 16, BLACK); 
    }
}
